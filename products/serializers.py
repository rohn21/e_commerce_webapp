...
class ProductRatingSerializer(serializers.ModelSerializer):
    ...
    def validate_rating(self, attrs):
        if attrs > 5:
            raise serializers.ValidationError("Please rate product in scale of 1 to 5.")
        return attrs
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['id', 'user', 'created_at', 'total_price', 'status', 'payment_status', 'order_items',
                  'applied_coupon', 'tracking_number']
...
class CouponSerializer(serializers.ModelSerializer):
    class Meta:
        model = Coupon
        fields = ['id', 'coupon_code', 'discount_value', 'max_discount', 'expires_at', 'is_active', 'discount_type',
                  'usage', 'created_at']
